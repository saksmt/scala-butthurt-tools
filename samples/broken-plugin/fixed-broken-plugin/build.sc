...
object fixedBrokenPlugin extends ScalaModule with PublishModule {
  ...
  def resources = T.sources(millSourcePath / os.up / "resources")
  ...
  override def pom = T {
    val originalPomFile = super.pom()
    val originalPom = XML.load(new BufferedInputStream(new FileInputStream(originalPomFile.path.toIO)))

    val sbtCompatiblePom = originalPom.copy(child = originalPom.child.map {
      case artifactId: Elem if artifactId.label == "artifactId" => <artifactId>{super.artifactName()}</artifactId>
      case name: Elem if name.label == "name" => <name>{super.artifactName()}</name>
      case other => other
    } ++ Seq(
      <properties>
        <scalaVersion>2.12</scalaVersion>
        <sbtVersion>{sbtVersionPrefix}</sbtVersion>
      </properties>
    ))

    val sbtCompatiblePomFile = T.ctx().dest / "pom.xml"

    os.write(sbtCompatiblePomFile, sbtCompatiblePom.toString())

    PathRef(sbtCompatiblePomFile)
  }
